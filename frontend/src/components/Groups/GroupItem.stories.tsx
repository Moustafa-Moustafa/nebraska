import { Meta, StoryFn } from '@storybook/react-vite';
import { MemoryRouter } from 'react-router';

import { PureGroupItem, PureGroupItemProps } from './GroupItem';

export default {
  title: 'groups/GroupItem',
  argTypes: {
    handleUpdateGroup: { action: 'handleUpdateGroup' },
    deleteGroup: { action: 'deleteGroup' },
  },
} as Meta;

const Template: StoryFn<PureGroupItemProps> = args => {
  return (
    <MemoryRouter>
      <PureGroupItem {...args} />
    </MemoryRouter>
  );
};

export const Group = {
  render: Template,

  args: {
    versionBreakdown: [],
    totalInstances: 2,
    group: {
      id: '11a585f6-9418-4df0-8863-78b2fd3240f8',
      name: 'Stable (ARM)',
      description: 'For production clusters (ARM)',
      created_ts: '2015-09-19T07:09:34.269062+02:00',
      rollout_in_progress: false,
      updates_disabled_due_to_failure: false,
      force_updates_enabled_ts: null,
      application_id: 'e96281a6-d1af-4bde-9a0a-97b76e56dc57',
      channel_id: '5dfe7b12-c94a-470d-a2b6-2eae78c5c9f5',
      policy_updates_enabled: true,
      policy_safe_mode: false,
      policy_office_hours: false,
      policy_timezone: 'Europe/Berlin',
      policy_period_interval: '1 minutes',
      policy_max_updates_per_period: 999999,
      policy_update_timeout: '60 minutes',
      channel: {
        id: '5dfe7b12-c94a-470d-a2b6-2eae78c5c9f5',
        name: 'stable',
        color: '#1458d6',
        created_ts: '2015-09-19T07:09:34.261241+02:00',
        application_id: 'e96281a6-d1af-4bde-9a0a-97b76e56dc57',
        package_id: null,
        package: null,
        arch: 2,
      },
      track: 'stable',
    },
  },
};

export const UpdatesAutoDisabled = {
  render: Template,

  args: {
    versionBreakdown: [],
    totalInstances: 2,
    group: {
      id: '11a585f6-9418-4df0-8863-78b2fd3240f8',
      name: 'Stable (ARM)',
      description: 'For production clusters (ARM)',
      created_ts: '2015-09-19T07:09:34.269062+02:00',
      rollout_in_progress: false,
      updates_disabled_due_to_failure: true,
      force_updates_enabled_ts: '2026-06-01T10:00:00.000000+00:00',
      application_id: 'e96281a6-d1af-4bde-9a0a-97b76e56dc57',
      channel_id: '5dfe7b12-c94a-470d-a2b6-2eae78c5c9f5',
      policy_updates_enabled: true,
      policy_safe_mode: true,
      policy_office_hours: false,
      policy_timezone: 'Europe/Berlin',
      policy_period_interval: '1 minutes',
      policy_max_updates_per_period: 999999,
      policy_update_timeout: '60 minutes',
      channel: {
        id: '5dfe7b12-c94a-470d-a2b6-2eae78c5c9f5',
        name: 'stable',
        color: '#1458d6',
        created_ts: '2015-09-19T07:09:34.261241+02:00',
        application_id: 'e96281a6-d1af-4bde-9a0a-97b76e56dc57',
        package_id: null,
        package: null,
        arch: 2,
      },
      track: 'stable',
    },
  },
};

export const Loading = {
  render: Template,

  args: {
    versionBreakdown: null,
    totalInstances: null,
    group: {
      id: '11a585f6-9418-4df0-8863-78b2fd3240f8',
      name: 'Stable (ARM)',
      description: 'For production clusters (ARM)',
      created_ts: '2015-09-19T07:09:34.269062+02:00',
      rollout_in_progress: false,
      updates_disabled_due_to_failure: false,
      force_updates_enabled_ts: null,
      application_id: 'e96281a6-d1af-4bde-9a0a-97b76e56dc57',
      channel_id: '5dfe7b12-c94a-470d-a2b6-2eae78c5c9f5',
      policy_updates_enabled: true,
      policy_safe_mode: false,
      policy_office_hours: false,
      policy_timezone: 'Europe/Berlin',
      policy_period_interval: '1 minutes',
      policy_max_updates_per_period: 999999,
      policy_update_timeout: '60 minutes',
      channel: {
        id: '5dfe7b12-c94a-470d-a2b6-2eae78c5c9f5',
        name: 'stable',
        color: '#1458d6',
        created_ts: '2015-09-19T07:09:34.261241+02:00',
        application_id: 'e96281a6-d1af-4bde-9a0a-97b76e56dc57',
        package_id: null,
        package: null,
        arch: 2,
      },
      track: 'stable',
    },
  },
};
