...
func (s *Service) AddApp(app *api.Application) (*api.Application, error) {
...
// channels from an existing application.
func (s *Service) AddAppCloning(app *api.Application, sourceAppID string) (*api.Application, error) {
...
func (s *Service) UpdateApp(app *api.Application) error {
	if err := validateProductID(app.ProductID); err != nil {
		return fmt.Errorf("cannot update application %v: %w", app.ID, err)
	}
