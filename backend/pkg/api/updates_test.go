...
func TestGetUpdatePackage(t *testing.T) {
	...
	_, err := a.GetUpdatePackage(Instance{ID: uuid.New().String(), IP: "10.0.0.1"}, NewInstanceApplication("invalidApplicationID", tGroup.ID, "1.0.0"))
	assert.Error(t, ErrInvalidApplicationOrGroup, err, "Invalid application id.")

	_, err = a.GetUpdatePackage(Instance{ID: uuid.New().String(), IP: "10.0.0.1"}, NewInstanceApplication(tApp.ID, "invalidGroupID", "1.0.0"))
	assert.Error(t, err, "Invalid group id.")

	_, err = a.GetUpdatePackage(Instance{ID: uuid.New().String(), IP: "10.0.0.1"}, NewInstanceApplication(uuid.New().String(), tGroup.ID, "1.0.0"))
	assert.Error(t, err, "Non existent application id.")

	_, err = a.GetUpdatePackage(Instance{ID: uuid.New().String(), IP: "10.0.0.1"}, NewInstanceApplication(tApp.ID, uuid.New().String(), "1.0.0"))
	assert.Error(t, err, "Non existent group id.")

	_, err = a.GetUpdatePackage(Instance{ID: uuid.New().String(), IP: "10.0.0.1"}, NewInstanceApplication(tApp.ID, tGroup2.ID, "1.0.0"))
	assert.Error(t, err, "Group doesn't belong to the application provided.")

	_, err = a.GetUpdatePackage(Instance{ID: uuid.New().String(), IP: "10.0.0.1"}, NewInstanceApplication(tApp2.ID, tGroup2.ID, "1.0.0"))
	assert.Equal(t, ErrNoPackageFound, err, "Group's channel has no package bound.")

	_, err = a.GetUpdatePackage(Instance{ID: uuid.New().String(), IP: "10.0.0.1"}, NewInstanceApplication(tApp.ID, tGroup.ID, "12.1.0"))
...
func TestGetUpdatePackage_GroupNoChannel(t *testing.T) {
	...
	_, _ = a.GetUpdatePackage(Instance{ID: uuid.New().String(), IP: "10.0.0.1"}, NewInstanceApplication(tApp.ID, tGroup.ID, "12.0.0"))
...
func TestGetUpdatePackage_MaxUpdatesPerPeriodLimitReached_LimitUpdated(t *testing.T) {
	...
	_, err := a.GetUpdatePackage(Instance{ID: instanceID, IP: "10.0.0.1"}, NewInstanceApplication(tApp.ID, tGroup.ID, "12.0.0"))
	assert.NoError(t, err)
...
func TestGetUpdatePackage_MaxUpdatesLimitsReached(t *testing.T) {
	...
	_, err := a.GetUpdatePackage(Instance{ID: newInstance1ID, IP: "10.0.0.1"}, NewInstanceApplication(tApp.ID, tGroup.ID, "12.0.0"))
	assert.NoError(t, err)

	_, err = a.GetUpdatePackage(Instance{ID: uuid.New().String(), IP: "10.0.0.2"}, NewInstanceApplication(tApp.ID, tGroup.ID, "12.0.0"))
	assert.NoError(t, err)

	_, err = a.GetUpdatePackage(Instance{ID: uuid.New().String(), IP: "10.0.0.3"}, NewInstanceApplication(tApp.ID, tGroup.ID, "12.0.0"))
...
func TestGetUpdatePackage_RolloutStats(t *testing.T) {
	...
	instance1, _ := a.RegisterInstance(Instance{ID: uuid.New().String(), IP: "10.0.0.1"}, NewInstanceApplication(tApp.ID, tGroup.ID, "1.0.0"))
	instance2, _ := a.RegisterInstance(Instance{ID: uuid.New().String(), IP: "10.0.0.1"}, NewInstanceApplication(tApp.ID, tGroup.ID, "1.0.0"))
	instance3, _ := a.RegisterInstance(Instance{ID: uuid.New().String(), IP: "10.0.0.1"}, NewInstanceApplication(tApp.ID, tGroup.ID, "1.0.0"))

	_, _ = a.GetUpdatePackage(Instance{ID: instance1.ID, IP: "10.0.0.1"}, NewInstanceApplication(tApp.ID, tGroup.ID, "12.0.0"))
	_, _ = a.GetUpdatePackage(Instance{ID: instance2.ID, IP: "10.0.0.2"}, NewInstanceApplication(tApp.ID, tGroup.ID, "12.0.0"))
	_, _ = a.GetUpdatePackage(Instance{ID: instance3.ID, IP: "10.0.0.3"}, NewInstanceApplication(tApp.ID, tGroup.ID, "12.0.0"))
...
func TestGetUpdatePackage_CompletionStats(t *testing.T) {
	...
	instance1, _ := a.RegisterInstance(Instance{ID: uuid.New().String(), IP: "10.0.0.1"}, NewInstanceApplication(tApp.ID, tGroup.ID, tPkg.Version))
	...
	_, err := a.GetUpdatePackage(Instance{ID: instance1.ID, IP: "10.0.0.1"}, NewInstanceApplication(tApp.ID, tGroup.ID, tPkg.Version))
	...
	_, err = a.RegisterInstance(Instance{ID: uuid.New().String(), IP: "10.0.0.1"}, NewInstanceApplication(tApp.ID, tGroup.ID, "0.1.0"))
	assert.NoError(t, err)
...
func TestGetUpdatePackage_UpdateInProgressOnInstance(t *testing.T) {
	...
	p1, err := a.GetUpdatePackage(Instance{ID: instanceID, IP: "10.0.0.1"}, NewInstanceApplication(tApp.ID, tGroup.ID, "12.0.0"))
	assert.NoError(t, err)

	p2, err := a.GetUpdatePackage(Instance{ID: instanceID, IP: "10.0.0.1"}, NewInstanceApplication(tApp.ID, tGroup.ID, "12.0.0"))
	assert.NoError(t, err)
...
func TestGetUpdatePackage_CheckVersionForGrantedUpdate(t *testing.T) {
	...
	_, err = a.GetUpdatePackage(Instance{ID: instanceID, IP: "10.0.0.1"}, NewInstanceApplication(tApp.ID, tGroup.ID, "12.1.0"))
...
func TestMultiStepUpdateProgression(t *testing.T) {
	...
	pkg, err := a.GetUpdatePackage(Instance{ID: instanceID, IP: "10.0.0.1"}, NewInstanceApplication(appID, groupID, "1000.0.0"))
	assert.NoError(t, err)
	...
	_, err = a.GetUpdatePackage(Instance{ID: instanceID, IP: "10.0.0.1"}, NewInstanceApplication(appID, groupID, "2000.0.0"))
	...
	pkg, err = a.GetUpdatePackage(Instance{ID: instanceID, IP: "10.0.0.1"}, NewInstanceApplication(appID, groupID, "2000.0.0"))
	assert.NoError(t, err)
	assert.Equal(t, "2500.0.0", pkg.Version, "Should get second floor")

	// Step 5: Instance updates to 2500 → should complete
	_, err = a.GetUpdatePackage(Instance{ID: instanceID, IP: "10.0.0.1"}, NewInstanceApplication(appID, groupID, "2500.0.0"))
	assert.Equal(t, ErrNoUpdatePackageAvailable, err)

	// Step 6: Instance at 2500 → should get target (3000)
	pkg, err = a.GetUpdatePackage(Instance{ID: instanceID, IP: "10.0.0.1"}, NewInstanceApplication(appID, groupID, "2500.0.0"))
	assert.NoError(t, err)
	assert.Equal(t, "3000.0.0", pkg.Version, "Should get target after all floors")

	// Step 7: Instance updates to 3000 → should complete
	_, err = a.GetUpdatePackage(Instance{ID: instanceID, IP: "10.0.0.1"}, NewInstanceApplication(appID, groupID, "3000.0.0"))
...
func TestAlreadyGrantedWithoutLastUpdateVersion(t *testing.T) {
	...
	// Get initial update
	pkg, err := a.GetUpdatePackage(Instance{ID: instanceID, IP: "10.0.0.1"}, NewInstanceApplication(setup.AppID, setup.Group.ID, "1000.0.0"))
	assert.NoError(t, err)
	...
	_, err = a.GetUpdatePackage(Instance{ID: instanceID, IP: "10.0.0.1"}, NewInstanceApplication(setup.AppID, setup.Group.ID, "1000.0.0"))
...
func TestSafetyRulesValidation(t *testing.T) {
	...
	_, err := a.RegisterInstance(Instance{ID: "safety-instance", IP: "10.0.0.1"}, NewInstanceApplication(setup.AppID, setup.Group.ID, "500.0.0"))
	assert.NoError(t, err)

	t.Run("floor_never_blacklisted_for_own_channel", func(t *testing.T) {
		// Get update should work normally
		pkg, err := a.GetUpdatePackage(Instance{ID: "safety-instance", IP: "10.0.0.1"}, NewInstanceApplication(setup.AppID, setup.Group.ID, "500.0.0"))
		assert.NoError(t, err)
