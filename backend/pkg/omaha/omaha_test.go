...
func TestInvalidRequests(t *testing.T) {
	a := newForTest(t)
	defer a.Close()
	adminSvc := admin.NewService(a.DB())
	runtimeSvc := apiruntime.NewService(a.DB(), true)
	h := NewHandler(runtimeSvc)

	tTeam, _ := adminSvc.AddTeam(&api.Team{Name: "test_team"})
	tApp, _ := adminSvc.AddApp(&api.Application{Name: "test_app", Description: "Test app", TeamID: tTeam.ID})
	tPkg, _ := adminSvc.AddPackage(&api.Package{Type: api.PkgTypeFlatcar, URL: "http://sample.url/pkg", Version: "640.0.0", ApplicationID: tApp.ID, Arch: api.ArchAMD64})
	tChannel, _ := adminSvc.AddChannel(&api.Channel{Name: "test_channel", Color: "blue", ApplicationID: tApp.ID, PackageID: null.StringFrom(tPkg.ID), Arch: api.ArchAMD64})
	tGroup, _ := adminSvc.AddGroup(&api.Group{Name: "test_group", ApplicationID: tApp.ID, ChannelID: null.StringFrom(tChannel.ID), PolicyUpdatesEnabled: true, PolicySafeMode: true, PolicyPeriodInterval: "15 minutes", PolicyMaxUpdatesPerPeriod: 2, PolicyUpdateTimeout: "60 minutes"})
...
func TestAppNoUpdateForAppWithChannelAndPackageName(t *testing.T) {
	a := newForTest(t)
	defer a.Close()
	h := NewHandler(apiruntime.NewService(a.DB(), true))

	tAppFlatcar, _ := queries(a).GetApp(flatcarAppID)
	tPkgFlatcar640, _ := admin.NewService(a.DB()).AddPackage(&api.Package{Type: api.PkgTypeFlatcar, URL: "http://sample.url/pkg", Version: "640.0.0", ApplicationID: tAppFlatcar.ID, Arch: api.ArchAMD64})
	tChannel, _ := admin.NewService(a.DB()).AddChannel(&api.Channel{Name: "mychannel", Color: "white", ApplicationID: tAppFlatcar.ID, PackageID: null.StringFrom(tPkgFlatcar640.ID), Arch: api.ArchAMD64})
	tGroup, _ := admin.NewService(a.DB()).AddGroup(&api.Group{Name: "Production", ApplicationID: tAppFlatcar.ID, ChannelID: null.StringFrom(tChannel.ID), PolicyUpdatesEnabled: true, PolicySafeMode: true, PolicyPeriodInterval: "15 minutes", PolicyMaxUpdatesPerPeriod: 2, PolicyUpdateTimeout: "60 minutes"})
...
func TestAppUpdateForAppWithChannelAndPackageName(t *testing.T) {
	...
	tPkgFlatcar640, _ := admin.NewService(a.DB()).AddPackage(&api.Package{Type: api.PkgTypeFlatcar, URL: "http://sample.url/pkg", Filename: null.StringFrom(tFilenameFlatcar), Version: "99640.0.0", ApplicationID: tAppFlatcar.ID, Arch: api.ArchAMD64})
	tChannel, _ := admin.NewService(a.DB()).AddChannel(&api.Channel{Name: "mychannel", Color: "white", ApplicationID: tAppFlatcar.ID, PackageID: null.StringFrom(tPkgFlatcar640.ID), Arch: api.ArchAMD64})
	tGroup, _ := admin.NewService(a.DB()).AddGroup(&api.Group{Name: "Production", ApplicationID: tAppFlatcar.ID, ChannelID: null.StringFrom(tChannel.ID), PolicyUpdatesEnabled: true, PolicySafeMode: true, PolicyPeriodInterval: "15 minutes", PolicyMaxUpdatesPerPeriod: 2, PolicyUpdateTimeout: "60 minutes"})
	flatcarAction, _ := admin.NewService(a.DB()).AddFlatcarAction(&api.FlatcarAction{Event: "postinstall", Sha256: "fsdkjjfghsdakjfgaksdjfasd", PackageID: tPkgFlatcar640.ID})
...
func TestProductIDBasedRequest(t *testing.T) {
	...
	tTeam, _ := admin.NewService(a.DB()).AddTeam(&api.Team{Name: "test_team"})
	tApp, _ := admin.NewService(a.DB()).AddApp(&api.Application{Name: "test_app", Description: "Test app", TeamID: tTeam.ID, ProductID: null.StringFrom("io.flatcar.MyApp")})
	tPkg, _ := admin.NewService(a.DB()).AddPackage(&api.Package{Type: api.PkgTypeFlatcar, URL: "http://sample.url/pkg", Version: "640.0.0", ApplicationID: tApp.ID, Arch: api.ArchAMD64})
	tChannel, _ := admin.NewService(a.DB()).AddChannel(&api.Channel{Name: "test_channel", Color: "blue", ApplicationID: tApp.ID, PackageID: null.StringFrom(tPkg.ID), Arch: api.ArchAMD64})
	tGroup, _ := admin.NewService(a.DB()).AddGroup(&api.Group{Name: "test_group", ApplicationID: tApp.ID, ChannelID: null.StringFrom(tChannel.ID), PolicyUpdatesEnabled: true, PolicySafeMode: false, PolicyPeriodInterval: "15 minutes", PolicyMaxUpdatesPerPeriod: 4, PolicyUpdateTimeout: "60 minutes"})
...
func TestMultiPackageResponse(t *testing.T) {
	...
	tTeam, _ := admin.NewService(a.DB()).AddTeam(&api.Team{Name: "test_team"})
	tApp, _ := admin.NewService(a.DB()).AddApp(&api.Application{Name: "test_app", Description: "Test app", TeamID: tTeam.ID, ProductID: null.StringFrom("io.flatcar.MyApp")})
	tPkg, _ := admin.NewService(a.DB()).AddPackage(&api.Package{Type: api.PkgTypeFlatcar, URL: "http://sample.url/pkg", Version: "640.0.0", ApplicationID: tApp.ID, Arch: api.ArchAMD64, ExtraFiles: extraFiles})
	tChannel, _ := admin.NewService(a.DB()).AddChannel(&api.Channel{Name: "test_channel", Color: "blue", ApplicationID: tApp.ID, PackageID: null.StringFrom(tPkg.ID), Arch: api.ArchAMD64})
	tGroup, _ := admin.NewService(a.DB()).AddGroup(&api.Group{Name: "test_group", ApplicationID: tApp.ID, ChannelID: null.StringFrom(tChannel.ID), PolicyUpdatesEnabled: true, PolicySafeMode: false, PolicyPeriodInterval: "15 minutes", PolicyMaxUpdatesPerPeriod: 100, PolicyUpdateTimeout: "60 minutes"})
	_, err := admin.NewService(a.DB()).AddFlatcarAction(&api.FlatcarAction{Event: "postinstall", Sha256: "fsdkjjfghsdakjfgaksdjfasd", PackageID: tPkg.ID})
